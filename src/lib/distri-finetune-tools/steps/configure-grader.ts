  ...
  try {
    ...
    if (dataset?.backendDatasetId) {
      try {
        ...
        // Don't fail the tool - local save succeeded
      }
    }
...
export const configureGraderTool: DistriFnTool = {
  ...
  description:
    "Configure the JavaScript evaluation script for RFT. The script should define an evaluate(input, output) function that returns { score, reasoning }.",
  type: "function",
  parameters: {
    type: "object",
    properties: {
      ...
      script: {
        type: "string",
        description:
          "JavaScript code defining an evaluate(input, output) function that returns { score: number, reasoning: string }.",
      },
    },
