...
export const getDatasetRecordsHandler: ToolHandler = async (params) => {
  try {
    ...
    return {
      ...
      offset: off,
      has_more: hasMore,
      // Hint to agent: use get_workflow_state for counts, this is just for sampling content
      note: hasMore ? `Showing ${paginatedRecords.length} of ${records.length} records. Use get_workflow_state for full counts - do NOT paginate through all records.` : undefined,
    };
  } catch (error) {
...
export const getDatasetRecordsTool: DistriFnTool = {
  name: 'get_workflow_records',
  description: 'Get sample records to preview content. Use limit=10-20 for representative samples. Do NOT paginate through all records - use get_workflow_state for counts.',
  type: 'function',
  parameters: {
    type: 'object',
    properties: {
