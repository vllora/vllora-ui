 ...
 */
function extractToolCalls(attr: Record<string, unknown>): Array<{ name: string; id?: string }> {
  ...
  if (attr['tool.name']) {
    toolCalls.push({ name: attr['tool.name'] as string });
  }
 ...
 */
function extractSpanSummary(span: Span): SpanSummary {
  ...
  const usage = attr.usage as Record<string, unknown> | undefined;
