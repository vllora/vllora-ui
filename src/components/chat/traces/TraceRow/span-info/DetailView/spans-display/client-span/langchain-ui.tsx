import { BookmarkIcon, BoxIcon, CaptionsIcon, FileJson } from "lucide-react";
...
export const LangchainInfoUIDisplay = ({ span }: { span: Span }) => {
    ...
    let metadata_value = attribute?.['metadata'] || undefined;
    let metadata_value_json = metadata_value ? tryParseJson(metadata_value) : null;
    ...
    let filterOutAttributes = ['openinference.span.kind', 'tool.description', 'metadata', 'langdb_client_name', 'langdb.client_name', 'llm.model_name', 'input.mime_type', 'output.mime_type', 'input.value', 'output.value'];
    ...
    return (
        <>
            ...
            {tool_descriptions && (
                <div className={`flex items-center justify-between px-3 py-2 border-b border-border`}>
                    <pre className="text-xs text-gray-400 font-mono whitespace-pre-wrap">{tool_descriptions}</pre>
                </div>
            )}
            {model_name && (
                <div className={`flex items-center justify-between px-3 py-2 border-b border-border`}>
            ...
            {metadata_value && (
                <div className={`flex items-center justify-between px-3 py-2 border-b border-border`}>
                    <div className="flex items-center gap-2">
                        <CaptionsIcon className="h-3.5 w-3.5 text-white" />
                        <span className="text-xs text-white w-[65px]">Metadata:</span>
            ...
            {metadata_value_json ? (
                <div className={`flex items-center justify-between px-3 py-2 border-b border-border`}>
                    <JsonViewer data={metadata_value_json} style={{
                        fontSize: '10px'
                    }} />
                </div>
            ) : (
               metadata_value ? <div className={`flex items-center justify-between px-3 py-2 border-b border-border`}>
                    <div className="p-3 bg-[#0d0d0d]">
                        <pre className="text-xs text-white font-mono whitespace-pre-wrap">{metadata_value}</pre>
                    </div>
                </div> : <></>
            )}
