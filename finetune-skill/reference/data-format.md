...
| `ground_truth` | No | Concise excerpt from the source material containing the information needed to answer this record's question. When present, the LLM-as-judge grader uses it to verify factual accuracy of model responses. Generated by `generate_records.py` (disable with `--no-ground-truth`). |
