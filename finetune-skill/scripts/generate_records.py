...
Return JSON: {{"items": [{{"prompt": "the question", "ground_truth": "relevant source excerpt"}}, ...]}}"""
    ...
    for i, item in enumerate(items):
        ...
        # Compose messages — optionally embed source context for extraction tasks.
        #
        # Architecture constraint: the system prompt is composed per-topic (root persona
        # + ancestor segments + leaf focus) and shared by all records in a topic. Source
        # context is per-record — each question references specific document sections.
        #
        # For extraction tasks, the ground_truth field (generated by the LLM alongside
        # each question) contains the relevant source excerpt for that specific question.
        # We embed it in the user message as a natural "here's the document, now answer"
        # pattern — this matches how real users interact in RAG-style workflows:
        #   User: "Here is a section from the 10-K filing:\n[excerpt]\n\nWhat was the revenue?"
        #
        # For non-extraction tasks, omit source context — the user message is just the question.
        ground_truth = item.get("ground_truth", "")
        if embed_source and ground_truth and ground_truth.strip():
            user_content = (
                f"Here is the relevant section from the source document:\n\n"
        ...
        # Store ground_truth as a record-level field for traceability (separate
        # from embedding it in the user message above for extraction tasks)
...
def main() -> None:
    ...
    parser.add_argument("--embed-source-context", action="store_true",
                        help="Embed per-question source excerpts (from ground_truth) into the "
                             "user message as 'Here is the relevant section... [excerpt] ... [question]'. "
                             "System prompt (topic hierarchy) stays unchanged. REQUIRED for extraction "
                             "tasks (document analysis, 10-K filing extraction) where the model needs "
                             "source material to extract from. NOT needed for knowledge/reasoning/style tasks.")
    ...
    common_kwargs = dict(
        ...
        embed_source=args.embed_source_context,
    )
